-- ============================================================
-- V10 — Data Collection Layer
-- Sales Transactions, Expenses, Inventory, Product Catalogue
-- ============================================================

-- ── Product Catalogue ─────────────────────────────────────────
-- Normalised product list per merchant
CREATE TABLE products (
                          id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                          merchant_profile_id UUID NOT NULL REFERENCES merchant_profiles(id) ON DELETE CASCADE,
                          name              VARCHAR(200) NOT NULL,
                          category          VARCHAR(100),
                          unit              VARCHAR(50)  DEFAULT 'piece',  -- piece, bag, litre, kg, metre, etc.
                          cost_price        NUMERIC(15,2),                  -- purchase price (COGS)
                          selling_price     NUMERIC(15,2),                  -- default selling price
                          current_stock     NUMERIC(15,2) DEFAULT 0,
                          reorder_level     NUMERIC(15,2) DEFAULT 0,        -- triggers low-stock alert
...
CREATE TABLE sales_transactions (
                                    ...
                                    product_name        VARCHAR(200) NOT NULL,   -- denormalised — survives product deletion
                                    quantity            NUMERIC(15,2) NOT NULL CHECK (quantity > 0),
                                    unit_price          NUMERIC(15,2) NOT NULL CHECK (unit_price >= 0),
                                    total_amount        NUMERIC(15,2) GENERATED ALWAYS AS (quantity * unit_price) STORED,
                                    cost_price          NUMERIC(15,2),            -- COGS at time of sale (for margin calc)
                                    transaction_date    DATE         NOT NULL,
                                    payment_method      VARCHAR(50)  DEFAULT 'cash',  -- cash, transfer, pos, credit
                                    notes               TEXT,
                                    source              VARCHAR(20)  DEFAULT 'manual', -- manual, csv, whatsapp, sms
                                    created_at          TIMESTAMP    NOT NULL DEFAULT NOW()
);
...
CREATE INDEX idx_sales_month         ON sales_transactions(merchant_profile_id, DATE_TRUNC('month', transaction_date));

-- ── Expense Records ───────────────────────────────────────────
CREATE TABLE expense_records (
                                 id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                                 merchant_profile_id UUID         NOT NULL REFERENCES merchant_profiles(id) ON DELETE CASCADE,
                                 category            VARCHAR(100) NOT NULL,  -- rent, salary, transport, restocking, utilities, misc
...
CREATE INDEX idx_expenses_month      ON expense_records(merchant_profile_id, DATE_TRUNC('month', expense_date));

-- ── Inventory Snapshots ───────────────────────────────────────
-- Point-in-time stock count records (not current state — that is on products table)
CREATE TABLE inventory_snapshots (
                                     ...
                                     unit_cost           NUMERIC(15,2),           -- cost per unit at snapshot time
...
-- ── CSV Upload Log ────────────────────────────────────────────
-- Track all bulk uploads for audit and re-processing
CREATE TABLE csv_upload_logs (
                                 id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                                 merchant_profile_id UUID         NOT NULL REFERENCES merchant_profiles(id) ON DELETE CASCADE,
                                 upload_type         VARCHAR(20)  NOT NULL,  -- sales, expenses, inventory
